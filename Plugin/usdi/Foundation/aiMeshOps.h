...
public:
    ...
    int weld(float3 *points, int count, const Compare& compare_op, const Welder& weld_op);
...
private:
    ...
    float3 *m_points = nullptr;
    int m_count = 0;
    int m_hash_size = 0;

    static const int NIL = -1;
    void prepare(float3 *points, int count);
    static uint32_t hash(const float3& v);
};
...
inline uint32_t MeshWelder::hash(const float3& value)
{
...
template<class Compare, class Welder>
inline int MeshWelder::weld(float3 *points, int count, const Compare & compare_op, const Welder & weld_op)
{
