...
template<> inline float$1 abcMin<float$1>(const float$1& a, const float$1& b)
{
    return float$1(std::min<float>(a.x, b.x), std::min<float>(a.y, b.y), std::min<float>(a.z, b.z));
}

template<> inline float$1 abcMax<float$1>(const float$1& a, const float$1& b)
{
    return float$1(std::max<float>(a.x, b.x), std::max<float>(a.y, b.y), std::max<float>(a.z, b.z));
}
...
inline abcBoxd abcGetMaxBounds(const Abc::IBox3dProperty& prop)
{
    ...
    bmin = (float$1)bounds.min;
    bmax = (float$1)bounds.max;

    for (size_t i = 1; i < n; ++i) {
        prop.get(bounds, aiIndexToSampleSelector((int)i));
        bmin = abcMin<float$1>(bmin, (float$1)bounds.min);
        bmax = abcMax<float$1>(bmax, (float$1)bounds.max);
    }
